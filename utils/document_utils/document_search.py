...
def show_search_result(top_k, content, filter_str, metrics, use_full_text_retrieval, only_show_content, show_ref_image,
    ...
    write_info(f'检索内容:{content}, 图片文件名:{file_name}, 参数：top_k:{top_k}, filter_str:{filter_str}, metrics:{metrics}, '
...
def retrieval_search_page():
    ...
    if st.button('文档检索'):
        show_search_result(top_k, content, filter_str, metrics, use_full_text_retrieval, only_show_content,
                           show_ref_image, file_name, file_bytes_data)
