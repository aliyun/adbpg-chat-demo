...
def check_create_metadata():
    ...
    try:
        ...
        st.session_state.create_dc_metadata_err_msg = f'Metadata is not format json：{str(e)}'
        return
    if 'metadata_invalid' in st.session_state:
        del st.session_state['metadata_invalid']
    st.session_state.create_dc_metadata_err_msg = ''
...
def do_create_document_collection():
    ...
    if not st.session_state.collection:
        st.session_state.create_dc_err_msg = 'Name can not be empty'
        return
    if st.session_state.collection in st.session_state.collections:
        st.session_state.create_dc_err_msg = f'Collection：{st.session_state.collection} already exists'
        return
    if 'metadata_invalid' in st.session_state:
        return
    st.session_state.create_dc_err_msg = ''
    try:
        st.session_state.create_dc_info_msg = f'Starting to create {st.session_state.collection}...'
        ...
        embedding_model = st.session_state.embedding_model.replace("(text)", "").replace("(image)", "")
        ...
        st.session_state.create_dc_info_msg = f'{st.session_state.collection} created successfully'
...
def show_create_document_collection_div(col_outer):
    if col_outer.button("Create document collection"):
    ...
    if "create_kb_btn" in st.session_state and st.session_state.create_kb_btn:
        horizontal_input('Name:').text_input(
            '', key='collection', max_chars=64, placeholder="can not be empty", label_visibility='collapsed',
            help='')
        horizontal_input('EmbeddingModel:').selectbox(
            '', key='embedding_model', options=['text-embedding-v1(text)', 'text-embedding-v2(text)', 'm3e-base(text)', 'm3e-small(text)',
                                                'text2vec(text)', 'multimodal-embedding-one-peace-v1(image)', 'clip-vit-b-32(image)',
                                                'clip-vit-b-16(image)', 'clip-vit-l-14(image)', 'clip-vit-l-14-336px(image)',
                                                'clip-rn50(image)', 'clip-rn101(image)', 'clip-rn50x4(image)', 'clip-rn50x16(image)',
                                                'clip-rn50x64(image)', ], label_visibility='collapsed')
        horizontal_input('Metadata:').text_area(
            '', key='metadata', placeholder='json format', label_visibility='collapsed', on_change=check_create_metadata,
            help='')
        write_error(st.session_state.create_dc_metadata_err_msg)
        horizontal_input('FullTextRetrievalFields:').multiselect(
            '', key='full_text_retrieval_fields', label_visibility='collapsed',
            options=st.session_state.full_text_retrieval_fields_opts)
        horizontal_input('Parser:').text_input(
            '', key='parser', value='zh_cn', label_visibility='collapsed', help='')
        ...
        horizontal_input('Hnsw_M:').number_input(
            '', value=64, key='hnsw_m', label_visibility='collapsed', help='')
