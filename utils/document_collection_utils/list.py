...
def show_list_document_collections_table():
    ...
    fields = ["Index", 'Name', 'EmbeddingModel', 'Dimension', 'Metrics', 'Ops']
    ...
    for i, dc in enumerate(st.session_state.document_collections):
        ...
        col6.button('Detail', key=f'detail_{i}_{st.session_state.page_index}',
                     on_click=show_document_collection_detail, args=[dc])
        col7.button('Enter', key=f'in_{i}_{st.session_state.page_index}',
                     on_click=check_in_document_collection, args=[dc])
        col8.button('Delete', key=f'delete_{i}_{st.session_state.page_index}',
