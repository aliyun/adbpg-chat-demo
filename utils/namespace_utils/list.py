...
def list_div(col_outer):
    ...
    # with col2:
    if col_outer.button('刷新Namespace列表'):
        try:
    ...
    for i, ns in enumerate(st.session_state.all_namespaces):
        ...
        col1.write(i)
        col2.write(ns)
        delete_modal = Modal(f"是否删除:{ns}", key=f"del_modal_{ns}", max_width=400)

        col3.button("进入文档库", key=f"checkin_{i}_{ns}_{st.session_state.page_index}",
                  on_click=check_in_document_collection, args=[ns])
        col4.button("删除", key=f"delete_{i}_{ns}_{st.session_state.page_index}",
