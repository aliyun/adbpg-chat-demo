...
class ADBPGClient:
    ...
    def create_document_collection(self,
        ...
        try:
            # 复制代码运行请自行打印 API 的返回值
            client.create_document_collection_with_options(create_document_collection_request, runtime)
            return Response(message="success", code=200)
        except Exception as error:
            # 此处仅做打印展示，请谨慎对待异常处理，在工程项目中切勿直接忽略异常。
            # 错误 message
...
def retrieval(retrival: Retrieval, api_key: str = Depends(verify_api_key)):
    ...
    for item in res:
        ...
        reranker_req.append(item["Content"])
        record["title"] = item["FileName"]
        record["metadata"] = item["Metadata"]
        record["loadermetadata"] = item["LoaderMetadata"]
    ...
    return RetrivalResponse(records=records_after_reranker)
...
if __name__ == '__main__':
    uvicorn.run(
        app="adbpg:app",  # 指向 FastAPI 实例（main.py 中的 app）
